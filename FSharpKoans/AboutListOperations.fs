    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
           let rec recLen xs count =
              match xs with
              |[] -> count
              |_::remeinder -> recLen remeinder (count + 1)
           recLen xs 0
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            let rec revList xs rList=
                match xs with
                |[] -> rList
                |a::rem -> revList rem (a::rList)
            revList xs []
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec revList xs rList=
                match xs with
                |[] -> rList
                |a::rem -> revList rem (rList@[a*2])
            revList xs [] // write a function which doubles each element
